// User Chat Manager - Para Profesores y Alumnos con Socket.IO
class UserChatManager {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data && result.data.conversacion) {
          ...
          // Unirse automáticamente a la conversación vía Socket.IO para recibir notificaciones
          if (this.socket && this.socket.connected) {
            console.log('🔌 Uniéndose automáticamente a conversación:', conversacion.id_conversacion);
            this.socket.emit('join_conversation', { id_conversacion: conversacion.id_conversacion });
          } else {
  ...
  async selectConversation(id) {
    ...
    // Unirse a la conversación vía Socket.IO
    if (this.socket && this.socket.connected) {
