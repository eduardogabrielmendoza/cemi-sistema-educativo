...
async function cargarAvatarUsuario() {
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/perfil/${idUsuario}`);
...
async function initCourseSelector() {
  try {
    ...
    if (isAdminClassroom) {
      const res = await fetchWithAuth(`${API_URL}/classroom/admin/todos-cursos`);
      ...
      const res = await fetchWithAuth(`${API_URL}/classroom/clases/${tipo}/${userId}`);
      cursosDisponibles = await res.json();
    }
...
async function loadFeed() {
  try {
    ...
    if (isAdminClassroom) {
      if (cursoActivo !== null) {
        const res = await fetchWithAuth(`${API_URL}/classroom/anuncios/curso/${cursoActivo}`);
        const data = await res.json();
        anuncios = Array.isArray(data) ? data : (data.anuncios || []);
        console.log(` Admin: ${anuncios.length} anuncios del curso ${cursoActivo}`);
      } else {
        const res = await fetchWithAuth(`${API_URL}/classroom/admin/todos-anuncios`);
        const data = await res.json();
        anuncios = Array.isArray(data) ? data : (data.anuncios || []);
      ...
      if (cursoActivo !== null) {
        ...
        const res = await fetchWithAuth(`${API_URL}/classroom/anuncios/${tipo}/${userId}`);
        const data = await res.json();
        anuncios = Array.isArray(data) ? data : (data.anuncios || []);
      }
    }
...
async function loadTareas() {
  try {
    ...
    if (isAdminClassroom) {
      if (cursoActivo !== null) {
        const res = await fetchWithAuth(`${API_URL}/classroom/tareas/curso/${cursoActivo}/${tipo}/${userId}`);
        tareas = await res.json();
      } else {
        const res = await fetchWithAuth(`${API_URL}/classroom/admin/todas-tareas`);
        tareas = await res.json();
      ...
      if (cursoActivo !== null) {
        ...
        const res = await fetchWithAuth(`${API_URL}/classroom/tareas-lista/${tipo}/${userId}`);
        tareas = await res.json();
      }
    }
...
async function loadCalificaciones() {
  try {
    ...
    const res = await fetchWithAuth(`${API_URL}/classroom/calificaciones/alumno/${userId}`);
    const data = await res.json();
...
async function loadEstadisticas() {
  try {
    ...
    const res = await fetchWithAuth(`${API_URL}/classroom/estadisticas/profesor/${userId}`);
...
async function verDetalleCurso(idCurso) {
  ...
  try {
    ...
    const [resCurso, resAlumnos, resTareas] = await Promise.all([
      fetchWithAuth(`${API_URL}/classroom/clases/${tipo}/${userId}`),
      fetchWithAuth(`${API_URL}/cursos/${idCurso}/alumnos`),
      fetchWithAuth(`${API_URL}/classroom/tareas/curso/${idCurso}/${tipo}/${userId}`)
    ]);
...
async function abrirPerfilEspectador(idPersona, tipoUsuario) {
  try {
    const response = await fetchWithAuth(`${API_URL}/classroom/perfil/${idPersona}`);
    const data = await response.json();
...
async function verAlumnosCurso(idCurso) {
  try {
    const res = await fetchWithAuth(`${API_URL}/classroom/curso/${idCurso}/alumnos`);
      ...
      if (archivoCheckbox.checked) {
        ...
        if (tipoArchivo === 'url') {
          ...
          try {
            ...
            const uploadResponse = await fetchWithAuth(`${API_URL}/classroom/upload-archivo`, {
              method: 'POST',
              body: formData
            });
...
async function crearTarea(datos) {
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/tareas`, {
      method: 'POST',
      headers: {
...
async function eliminarTarea(idTarea, titulo) {
  ...
  if (result.isConfirmed) {
    try {
      const response = await fetchWithAuth(`${API_URL}/classroom/tareas/${idTarea}`, {
        method: 'DELETE'
      });
...
async function entregarTarea(idTarea, comentario, archivo) {
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/entregas`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function verDetalleEntrega(idTarea, idAlumno) {
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/entregas/${idTarea}/alumno/${idAlumno}`);
...
async function verEntregasTarea(idTarea) {
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/entregas/${idTarea}`);
...
window.calificarEntrega = async function(idEntrega) {
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/entrega/${idEntrega}`);
...
async function guardarCalificacion(idEntrega, calificacion, comentario) {
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/entregas/${idEntrega}/calificar`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
...
window.votarEncuesta = async function(idEncuesta, idOpcion, idAnuncio) {
  try {
    const response = await fetchWithAuth(`${API_URL}/classroom/encuestas/votar`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (response.ok) {
      const resEncuesta = await fetchWithAuth(`${API_URL}/classroom/encuestas/${idEncuesta}/${userId}`);
...
async function crearAnuncio(datos) {
  try {
    const response = await fetchWithAuth(`${API_URL}/classroom/anuncios`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function loadAnunciosProfesor() {
  try {
    const res = await fetchWithAuth(`${API_URL}/classroom/anuncios/profesor/${userId}`);
...
window.abrirAnuncio = async function(idAnuncio) {
  try {
    const resAnuncio = await fetchWithAuth(`${API_URL}/classroom/anuncio/${idAnuncio}/${userId}`);
    const anuncio = await resAnuncio.json();
    
    console.log(' Datos del anuncio:', anuncio);
    console.log('‍ ID Profesor:', anuncio.id_profesor);
    
    const resComentarios = await fetchWithAuth(`${API_URL}/classroom/comentarios/${idAnuncio}`);
      ...
      didOpen: () => {
        ...
        btnEnviar.onclick = async () => {
          ...
          try {
            ...
            const response = await fetchWithAuth(`${API_URL}/classroom/comentarios`, {
              method: 'POST',
...
async function cargarCalendario() {
  try {
    ...
    const resCalendario = await fetchWithAuth(`${API_URL}/classroom/calendario/${tipo}/${userId}/${calendarioActual.año}/${calendarioActual.mes}`);
    ...
    const resNotas = await fetchWithAuth(`${API_URL}/classroom/notas/${tipo}/${userId}/${calendarioActual.año}/${calendarioActual.mes}`);
...
async function crearNota(datos) {
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/notas`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function editarNota(idNota, fecha) {
  try {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const response = await fetchWithAuth(`${API_URL}/classroom/notas/${idNota}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
...
async function mostrarFormularioEvento() {
  const res = await fetchWithAuth(`${API_URL}/classroom/clases/profesor/${userId}`);
  const cursos = await res.json();
...
async function crearEvento(datos) {
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/calendario/eventos`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function cargarNotificaciones() {
  try {
    const tipo = userRol.toLowerCase() === 'profesor' ? 'profesor' : 'alumno';
    const res = await fetchWithAuth(`${API_URL}/notificaciones/${tipo}/${userId}?limit=20`);
...
async function actualizarContador() {
  try {
    const tipo = userRol.toLowerCase() === 'profesor' ? 'profesor' : 'alumno';
    const res = await fetchWithAuth(`${API_URL}/notificaciones/${tipo}/${userId}/sin-leer`);
    const data = await res.json();
...
async function clickNotificacion(idNotificacion, tipoNotificacion, idReferencia) {
  try {
    await fetchWithAuth(`${API_URL}/notificaciones/${idNotificacion}/marcar-leida`, {
      method: 'PUT'
    });
...
async function marcarTodasLeidas() {
  try {
    const tipo = userRol.toLowerCase() === 'profesor' ? 'profesor' : 'alumno';
    await fetchWithAuth(`${API_URL}/notificaciones/${tipo}/${userId}/marcar-todas-leidas`, {
      method: 'PUT'
    });
...
async function eliminarAnuncioAdmin(idAnuncio) {
  ...
  try {
    const res = await fetchWithAuth(`${API_URL}/classroom/anuncio/${idAnuncio}`, {
      method: 'DELETE'
    });
...
async function eliminarTareaAdmin(idTarea, titulo, nombreCurso) {
  ...
  try {
    const res = await fetchWithAuth(`${API_URL}/classroom/tarea/${idTarea}`, {
      method: 'DELETE'
    });
...
async function cambiarPasswordClassroom(event) {
  ...
  try {
    const API_URL = window.API_URL || 'http://localhost:3000/api';
    const response = await fetchWithAuth(`${API_URL}/auth/cambiar-password-classroom`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function loadRecursos() {
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/recursos/${tipo}/${userId}`);
    const data = await response.json();
...
async function handleSubirRecurso(e) {
  ...
  try {
    ...
    const response = await fetchWithAuth(`${API_URL}/classroom/recursos`, {
      method: 'POST',
      body: formData
    });
...
async function descargarRecurso(idRecurso, url) {
  try {
    if (url.includes('cloudinary.com')) {
      const response = await fetchWithAuth(`${API_URL}/classroom/recursos/${idRecurso}/download-url`);
      const data = await response.json();
    ...
    await fetchWithAuth(`${API_URL}/classroom/recursos/${idRecurso}/descarga`, {
      method: 'POST'
    });
...
async function eliminarRecurso(idRecurso) {
  ...
  try {
    const response = await fetchWithAuth(`${API_URL}/classroom/recursos/${idRecurso}`, {
      method: 'DELETE'
    });
